// Multi Area Conditioning Mask - JavaScript UI
// Updated: 2025-11-22
// Visual interface for mask-based regional conditioning (Flux, Chroma, SD3+)
...
function addMultiAreaConditioningMaskCanvas(node, app) {

	const widget = {
		type: "customCanvas",
		name: "MultiAreaConditioningMask-Canvas",
		get value() {
		...
		draw: function (ctx, node, widgetWidth, widgetY) {
			...
			// Get canvas dimensions from properties (synced from width/height inputs)
			...
			// Index widget is at position 2 (after width and height)
			const index = Math.round(node.widgets[2].value)
...
app.registerExtension({
	name: "Comfy.Davemane42.MultiAreaConditioningMask",
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		if (nodeData.name === "MultiAreaConditioningMask") {
			...
			nodeType.prototype.onNodeCreated = function () {
				...
				this.selected = false
				this.index = 2  // Index widget is now at position 2 (after width/height inputs)

				this.serialize_widgets = true;

				addMultiAreaConditioningMaskCanvas(this, app)
				...
				CUSTOM_INT(
					...
					function (v, _, node) {
						...
						// Widget indices are offset by 2 (width and height inputs come first)
				...
				// Sync canvas dimensions from input widgets
				...
				this.onExecuted = function(message) {
					...
					// Sync width/height from input widgets to properties for canvas rendering
					if (this.widgets && this.widgets.length >= 2) {
						// Width and height are first two widgets (inputs)
						if (this.widgets[0].value !== undefined) {
							this.properties["width"] = this.widgets[0].value;
						}
						if (this.widgets[1].value !== undefined) {
							this.properties["height"] = this.widgets[1].value;
						}
					}
				};
					...
					options.unshift(
						...
						},
						{
							...
							callback: () => {
								...
								for (let i = inputLength; i > index+1; i--) {
									swapInputs(this, i, i-1)
								}
								renameNodeInputs(this, "conditioning")
	...
	loadedGraphNode(node, _) {
		if (node.type === "MultiAreaConditioningMask") {
